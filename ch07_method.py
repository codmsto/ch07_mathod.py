...
#  2. 함수의 역할
#   + 하나의 함수에 휴사한 역할을 하는 코드만 포함
#   + 하나의 함수는 한 가지 기능만 명학히 정의
#   ex) 계산기(덧셈,뺄셈,나눗셈,곱셈)
#   -> 제곱 기능(곱셈 2번)
#   -> 곱셈() x 2번 사용 -> 재사용성 Up
...
#   + 함수에 전달되는 입력값
#   + parameter로 int, str, float, bool, list등 사용가능
...
#    -> 종속성: A 코드를 수정하면 B코드,C코드도 수정해야 하는 경우
...
#   + 함수 내의 변수는 자신이 속한 코드 블록이 종료되면 소면
